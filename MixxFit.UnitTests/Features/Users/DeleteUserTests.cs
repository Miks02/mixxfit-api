...
public class DeleteUserTests
{
    ...
    private const string OriginalEmail = "john@doe.com";
    ...
    public async Task Handle_WhenUserIsNull_ShouldReturnFailure()
    {
        ...
        _emailSenderMock.Verify(e => e.SendAccountDeletedEmailAsync(It.IsAny<string>()), Times.Never);
    }
    ...
    [Fact]
    public async Task Handle_WhenUserHasNoRelevantData_ShouldHardDeleteAndDeleteImage()
    {
        var user = CreateUser();
        var handler = CreateHandler(user);
    ...
    [Fact]
    public async Task Handle_WhenHardDeleteImageDeletionFails_ShouldReturnFailureAndNotDeleteUser()
    {
        _fileServiceMock
            .Setup(f => f.DeleteFile(It.IsAny<string>()))
            .ReturnsAsync(Result.Failure(new Error("File.DeleteFailed", "boom")));
        var handler = CreateHandler(CreateUser());
    ...
    [Fact]
    public async Task Handle_WhenAnonymizeImageDeletionFails_ShouldReturnFailureAndNotUpdateUser()
    {
        _fileServiceMock
            .Setup(f => f.DeleteFile(It.IsAny<string>()))
            .ReturnsAsync(Result.Failure(new Error("File.DeleteFailed", "boom")));
    ...
    [Fact]
    public async Task Handle_WhenHardDeleting_ShouldSendDeletedEmailToOriginalAddress()
    {
        var handler = CreateHandler(CreateUser());

        await handler.Handle(UserId);

        _emailSenderMock.Verify(e => e.SendAccountDeletedEmailAsync(OriginalEmail), Times.Once);
    }

    [Fact]
    public async Task Handle_WhenAnonymizing_ShouldSendDeletedEmailToOriginalAddress()
    {
        var handler = CreateHandler(CreateUser(), workouts: [new Workout { Id = 1, Name = "Push", OwnerId = UserId }]);
    ...
    [Fact]
    public async Task Handle_WhenDeletionFails_ShouldNotSendEmail()
    {
        _userManagerMock
            .Setup(m => m.DeleteAsync(It.IsAny<User>()))
            .ReturnsAsync(IdentityResult.Failed(new IdentityError { Code = "Unknown", Description = "boom" }));
        var handler = CreateHandler(CreateUser());
